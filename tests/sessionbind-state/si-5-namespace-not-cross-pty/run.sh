...
PPZ_IPC_SOCKET="$PPZ_DAEMON_A_SOCK" \
  ppz terminal share cindy -- sh -c '
    ...
    env -u PPZ_CURRENT_HANDLE -u PPZ_SESSION ppz status 2>&1 | grep -E "^namespace:" > /tmp/si-5-cindy.txt
...
PPZ_IPC_SOCKET="$PPZ_DAEMON_A_SOCK" \
  ppz terminal share bob -- sh -c '
    out=$(env -u PPZ_CURRENT_HANDLE -u PPZ_SESSION ppz status 2>&1 | grep -E "^namespace:" || true)
    if [ -n "$out" ]; then
      echo "$out" > /tmp/si-5-bob.txt
    else
