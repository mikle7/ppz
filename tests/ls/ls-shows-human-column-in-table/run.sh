#!/usr/bin/env bash
# `ppz ls` table layout: PIPE UNREAD BUFFERED LAST PAYLOAD HUMAN.
# HUMAN is rightmost — the username that owns the (source, pipe).
# alpha-primary→foo creates `chat`; the table includes a HUMAN column
...
# in (we want to see HUMAN as the last header), so use sed rather than
