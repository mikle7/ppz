#!/usr/bin/env bash
# Remote-created collared pipe in manifold "xyz": daemon A's ls view
...
# Daemon A's session stays at root — proves the cell tracks the pipe's
# manifold, not the listing session's namespace.
