#!/usr/bin/env bash
# When foo mints a new api key via POST /orgs/<name>/keys, the row that
# results in /orgs/<id>/keys carries `data-key-creator="foo"`. Seeded
...
curl_server "/orgs/$org_id/keys" -X POST --data-urlencode 'label=fresh-key' >/dev/null

# Count how many rows attribute to each user.
keys_html=$(curl_server "/orgs/$org_id/keys")
