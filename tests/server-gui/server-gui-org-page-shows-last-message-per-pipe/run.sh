...
# just broadcast. Column header is "Last Message" (not "Last Broadcast").
...
ppz_a broadcast -m "hello-broadcast" >/dev/null
...
echo "--- per-pipe rows: handle:pipe:RELATIVE:payload (broadcast + archive both populated) ---"
echo "$PAGE" \
