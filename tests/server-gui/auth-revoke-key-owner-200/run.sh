...
KEY_ID=$(curl_server "/accounts/$ORG_ID/keys" -b "$COOKIE_JAR" -s \
           | grep -oE 'data-key-id="[^"]+"' | head -1 \
           | sed -E 's/data-key-id="([^"]+)"/\1/')

echo "--- POST revoke as foo (owner of alpha) ---"
curl_server "/accounts/$ORG_ID/keys/$KEY_ID/revoke" -X POST -b "$COOKIE_JAR" \
    --max-redirs 0 -o /dev/null -w "status=%{http_code}\n" -s

echo "--- key state after revoke ---"
curl_server "/accounts/$ORG_ID/keys" -b "$COOKIE_JAR" -s \
