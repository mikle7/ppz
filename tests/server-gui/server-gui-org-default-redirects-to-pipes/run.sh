#!/usr/bin/env bash
# `/orgs/<id>` is now a router stub that 303-redirects to the Pipes
...
{ curl_server "/orgs/$org_id" --max-redirs 0 -o /dev/null -w 'http=%{http_code}\n' || true; }

echo "--- redirect target is the pipes tab ---"
{ curl_server "/orgs/$org_id" --max-redirs 0 -s -D - -o /dev/null || true; } \
  | grep -i '^location:' \
  | sed -E 's/\r$//' \
  | head -1
