...
# mode=internal). POST /accounts accepts an optional owner_user_id and
...
echo "--- POST /accounts with only a name → owner defaults to the authed session user ---"
curl_server "/accounts" -X POST --data-urlencode 'name=plain' \
  -o /dev/null -w 'http=%{http_code}\n'
PGPASSWORD=ppz psql -h postgres -U postgres -d ppz -tAc \
  "SELECT u.username FROM accounts o
     JOIN users u ON o.owner_user_id = u.id
