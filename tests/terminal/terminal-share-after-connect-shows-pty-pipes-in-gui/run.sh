#!/usr/bin/env bash
# Repro: `ppz connect foo` creates source 'foo' as kind=message (inbox
...
# `db.Source.Pipes()`) only renders foo:inbox. The user sees stdin
...
# so the org page renders four rows (inbox / stdctrl / stdin / stdout) for
