# Running a local ppz pipes server (self-hosting)
...
## Path C — Native binary + Postgres (verified)
...
```bash
ppz login http://localhost:8080 -apikey "$(cat <seed-dir>/key-alpha.txt)"   # (verified)
```
