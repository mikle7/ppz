...
func (c *csiEscape) parse() {
	...
	s := string(c.buf)
	c.args = c.args[:0]
	// Private-prefix CSI: ECMA-48 reserves ? < = > as "private" markers.
	// Upstream vt10x only honored '?', so e.g. kitty's `CSI < u` (pop
	// keyboard mode) was being mis-dispatched as `CSI u` (DECRC),
	// jumping the cursor back to the saved position. Treat all four as
	// private and let the dispatcher decide what to do with them.
	if s[0] == '?' || s[0] == '<' || s[0] == '=' || s[0] == '>' {
		c.priv = true
		s = s[1:]
	}
...
func (t *State) handleCSI() {
	...
	case 's': // DECSC - save cursor position (ANSI.SYS)
		// Skip when private — e.g. xterm `CSI > s` (modify other keys)
		// is not DECSC.
		if !c.priv {
			t.saveCursor()
		}
	case 'u': // DECRC - restore cursor position (ANSI.SYS)
		// Skip when private — kitty `CSI < u` (pop keyboard) and `CSI > u`
		// are not DECRC; mis-dispatching them silently teleports the
		// cursor and corrupts subsequent rendering.
		if !c.priv {
			t.restoreCursor()
		}
	}
